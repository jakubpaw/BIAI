...
namespace NeuralNetwork
{
    public class NeuralNetwork
    {
        ...
        public void TrainUsingRoot(double[] inputs, double[] targets)
        {
            var inputSignals = ConvertToMatrix(inputs);
            var targetSignals = ConvertToMatrix(targets);


            var hiddenOutputs = SquareRoor(_weightInputHidden * inputSignals);
            var finalOutputs = SquareRoor(_weightHiddenOutput * hiddenOutputs);
        ...
        public void TrainUsingSigmoid(double[] inputs, double[] targets)
        {
            var inputSignals = ConvertToMatrix(inputs);
            var targetSignals = ConvertToMatrix(targets);
        ...
        public void TrainUsingSinus(double[] inputs, double[] targets)
        {
            var inputSignals = ConvertToMatrix(inputs);
            var targetSignals = ConvertToMatrix(targets);


            var hiddenOutputs = Sinus(_weightInputHidden * inputSignals);
            var finalOutputs = Sinus(_weightHiddenOutput * hiddenOutputs);
        ...
        public double[] QueryUsingSigmoid(double[] inputs)
        {
            var inputSignals = ConvertToMatrix(inputs);
        ...
        public double[] QueryUsingSinus(double[] inputs)
        {
            var inputSignals = ConvertToMatrix(inputs);
        ...
        public double[] QueryUsingRoot(double[] inputs)
        {
            var inputSignals = ConvertToMatrix(inputs);
        ...
        private Matrix Sinus(Matrix matrix)
        {
            var newMatrix = Matrix.Create(matrix.Value.Length, matrix.Value[0].Length);
            ...
            for (var x = 0; x < matrix.Value.Length; x++)
            {
                for (var y = 0; y < matrix.Value[x].Length; y++)
                {
                    newMatrix.Value[x][y] = Math.Sin(matrix.Value[x][y]);
                }
            }
            return newMatrix;
        }

        private Matrix SquareRoor(Matrix matrix)
        {
            var newMatrix = Matrix.Create(matrix.Value.Length, matrix.Value[0].Length);
            ...
            for (var x = 0; x < matrix.Value.Length; x++)
            {
                for (var y = 0; y < matrix.Value[x].Length; y++)
                {
                    newMatrix.Value[x][y] = Math.Sqrt(matrix.Value[x][y]);
                }
            }
            return newMatrix;
        }
